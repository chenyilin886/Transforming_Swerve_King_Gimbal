 ...
 *
 *   摩擦轮使能独立于状态机：
 *     - 遥控模式：S1上 + S2上 且无急停/离线 → friction_enable=1
 ...
 *   STOP    ──(预留)──→ AUTO
 *
 * @note 摩擦轮使能独立于此状态机，遥控模式由 S1上 + S2上 控制，键鼠模式由 R 键锁存（见 updateFriction_）
 */
...
public:
    ...
    // === 摩擦轮控制字段 ===
    uint8_t    friction_enable;    ///< 摩擦轮使能(0=停, 1=转), 遥控由 S1上 + S2上 控制, 键鼠由 R 键锁存
     ...
     *   5. 摩擦轮速度环 PID 控制（遥控 S1上 + S2上 / 键鼠 R 键使能）
