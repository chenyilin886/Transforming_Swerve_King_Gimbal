...
1. DM4340 CAN 协议是与 DM4310 相同，仅参数不一样，4340：PMAX (最大位置控制范围): **12.5 rad**，VMAX (最大速度限制): **10.0 rad/s** (出厂默认读取值)，TMAX (最大扭矩限制): **28.0 N·m** (驱动器底层设定的安全软限幅)，目前只能得到这些参数
2. 控制模式RM2026 统一用MIT力矩模式
3. 我是重新建立了一个cubemx工程，希望从零开始参考参考工程开发
4. Fold 使用普通 DM Motor。不新增协议。仅新增一个 CAN ID。ID：0x03。与 Yaw、Pitch 保持一致。